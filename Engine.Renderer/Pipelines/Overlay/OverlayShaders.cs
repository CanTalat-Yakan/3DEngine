...
public static class OverlayShaders
{
    ...
    public static void EnsureLoaded()
    {
        ...
        Logger.Info("Loading overlay shaders...");
        ...
        if (File.Exists(vertSpvPath) && File.Exists(fragSpvPath))
        {
            ...
            _loaded = true;
            Logger.Info($"Overlay shaders loaded from SPIR-V (vertex={Vertex.Length} bytes, fragment={Fragment.Length} bytes).");
            return;
        }
        ...
        if (File.Exists(vertGlslPath) && File.Exists(fragGlslPath))
        {
            ...
            if (File.Exists(vertSpvPath) && File.Exists(fragSpvPath))
            {
                ...
                Fragment = File.ReadAllBytes(fragSpvPath);
                _loaded = true;
                Logger.Info($"Overlay shaders compiled and loaded (vertex={Vertex.Length} bytes, fragment={Fragment.Length} bytes).");
                return;
            }
