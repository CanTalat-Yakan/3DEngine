...
public static class ImGuiShaders
{
    ...
    public static void EnsureLoaded()
    {
        ...
        if (File.Exists(vertSpvPath) && File.Exists(fragSpvPath))
        {
            ...
            _loaded = true;
            Logger.Info($"ImGui shaders loaded from SPIR-V (vertex={Vertex.Length} bytes, fragment={Fragment.Length} bytes).");
            return;
        }
        ...
        if (File.Exists(vertGlslPath) && File.Exists(fragGlslPath))
        {
            ...
            if (File.Exists(vertSpvPath) && File.Exists(fragSpvPath))
            {
                ...
                Fragment = File.ReadAllBytes(fragSpvPath);
                _loaded = true;
                Logger.Info($"ImGui shaders compiled and loaded (vertex={Vertex.Length} bytes, fragment={Fragment.Length} bytes).");
                return;
            }
        ...
        throw new InvalidOperationException("ImGui shaders could not be loaded. Ensure SPIR-V files exist or glslc is available.");
    }
