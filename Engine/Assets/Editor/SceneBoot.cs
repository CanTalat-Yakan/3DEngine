...
internal sealed class SceneBoot : EditorComponent, IHide
{
    ...
    public override void OnAwake()
    {
        ...
        SceneCamera = SceneManager.MainScene.CreateCamera("Camera");
        ...
        var defaultSky = SceneManager.MainScene
            .CreateEntity()
    ...
    {
        var exampleCamera = SceneManager.MainScene.CreateCamera("Camera", Tags.MainCamera.ToString()).Entity;
        exampleCamera.Transform.LocalPosition = new(3, 4, 5);
        exampleCamera.Transform.EulerAngles = new(35, -150, 0);

        Cubes = SceneManager.MainScene.CreateEntity(null, "Cubes");
        SceneManager.MainScene.CreatePrimitive(PrimitiveTypes.Cube, parent: Cubes);
    ...
    public override void OnUpdate()
    {
        ...
        if (Input.GetKey(Key.C, InputState.Pressed) && ViewportController.ViewportFocused)
        {
            ...
            {
                var newCube = SceneManager.MainScene.CreatePrimitive(PrimitiveTypes.Cube, parent: Cubes, hide: true).Entity;
        ...
        if (Input.GetKey(Key.E, InputState.Pressed) && ViewportController.ViewportFocused)
        {
            ...
            for (int i = 0; i < 1000; i++)
                SceneManager.MainScene.CreateEntity(hide: true).AddComponent<HoverEffect>();
        }
    }
}
